...
import { DeleteOutlined, EditOutlined, SendOutlined,InfoCircleOutlined } from '@ant-design/icons'
import { fetchTaskData, fetchUserData, fetchprojectData } from '../redux/features/asyncThunkApi';
import Loading from '../Loading';

const AllTasks = ({ proid, projectTasks }) => {
    ...
    return (
        <div className='pt-5' >
            {loading && <Loading />}
            <span className='text-slate-700 text-2xl pt-44 px-1'>ALL TASKS</span>
            <hr className='h-1 w-full bg-slate-600 rounded-b-lg mb-11' />
            <i className='text-slate-400 text-sm font-thin'><InfoCircleOutlined className='px-2'/>go to projects inorder to add tasks</i>
